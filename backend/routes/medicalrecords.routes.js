  ...
  try {
    const result = await exec(`
      SELECT 
        r.RECORDID,
        r.VISITDATE,
        (SELECT PATIENTIC FROM APPOINTMENTS WHERE APPOINTMENTID = r.APPOINTMENTID) as PATIENTIC,
        (SELECT FIRSTNAME || ' ' || LASTNAME FROM PATIENTS 
         WHERE PATIENTIC = (SELECT PATIENTIC FROM APPOINTMENTS WHERE APPOINTMENTID = r.APPOINTMENTID)) as PATIENT_NAME,
        (SELECT DOCTORID FROM APPOINTMENTS WHERE APPOINTMENTID = r.APPOINTMENTID) as DOCTORID,
        (SELECT FIRSTNAME || ' ' || LASTNAME FROM DOCTORS 
         WHERE DOCTORID = (SELECT DOCTORID FROM APPOINTMENTS WHERE APPOINTMENTID = r.APPOINTMENTID)) as DOCTOR_NAME,
        r.SYMPTOM,
        r.DIAGNOSIS,
        r.APPOINTMENTID
