  ...
  try {
    const result = await exec(`
      SELECT 
        ...
        p.RECORDID,
        p.INSTRUCTION,
        (SELECT VISITDATE FROM MEDICALRECORD WHERE RECORDID = p.RECORDID) as VISITDATE,
        (SELECT PATIENTIC FROM APPOINTMENTS WHERE APPOINTMENTID = 
         (SELECT APPOINTMENTID FROM MEDICALRECORD WHERE RECORDID = p.RECORDID)) as PATIENTIC,
        (SELECT FIRSTNAME || ' ' || LASTNAME FROM PATIENTS WHERE PATIENTIC = 
         (SELECT PATIENTIC FROM APPOINTMENTS WHERE APPOINTMENTID = 
          (SELECT APPOINTMENTID FROM MEDICALRECORD WHERE RECORDID = p.RECORDID))) as PATIENT_NAME,
        (SELECT DOCTORID FROM APPOINTMENTS WHERE APPOINTMENTID = 
         (SELECT APPOINTMENTID FROM MEDICALRECORD WHERE RECORDID = p.RECORDID)) as DOCTORID,
        (SELECT FIRSTNAME || ' ' || LASTNAME FROM DOCTORS WHERE DOCTORID = 
         (SELECT DOCTORID FROM APPOINTMENTS WHERE APPOINTMENTID = 
          (SELECT APPOINTMENTID FROM MEDICALRECORD WHERE RECORDID = p.RECORDID))) as DOCTOR_NAME,
        (SELECT DIAGNOSIS FROM MEDICALRECORD WHERE RECORDID = p.RECORDID) as DIAGNOSIS
      FROM PRESCRIPTION p
      ORDER BY (SELECT VISITDATE FROM MEDICALRECORD WHERE RECORDID = p.RECORDID) DESC
    `);
      ...
      `SELECT 
        ...
        (SELECT FIRSTNAME || ' ' || LASTNAME FROM DOCTORS WHERE DOCTORID = 
         (SELECT DOCTORID FROM APPOINTMENTS WHERE APPOINTMENTID = 
          (SELECT APPOINTMENTID FROM MEDICALRECORD WHERE RECORDID = p.RECORDID))) as DOCTOR_NAME,
        (SELECT DIAGNOSIS FROM MEDICALRECORD WHERE RECORDID = p.RECORDID) as DIAGNOSIS,
        (SELECT SYMPTOM FROM MEDICALRECORD WHERE RECORDID = p.RECORDID) as SYMPTOM
      FROM PRESCRIPTION p
